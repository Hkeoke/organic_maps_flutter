...
public final class OrganicMaps implements DefaultLifecycleObserver
{
  ...
  private boolean initNativeFramework(@NonNull Runnable onComplete)
  {
    ...
    // Crear un wrapper del callback que inicialice todo DESPUÉS de que el framework nativo esté listo
    Runnable wrappedCallback = new Runnable() {
      @Override
      public void run() {
        // Inicializar todo lo que depende del framework nativo
        initNativeStrings();
        ...
        ProcessLifecycleOwner.get().getLifecycle().addObserver(OrganicMaps.this);

        Logger.i(TAG, "Framework fully initialized");
        mFrameworkInitialized = true;
        
        // Llamar al callback original del usuario
        if (onComplete != null) {
          onComplete.run();
        }
      }
    ...
    nativeInitFramework(wrappedCallback);
